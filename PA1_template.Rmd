...
g<-g+ scale_colour_discrete(name  ="Weekday or weekend",
                          ...
                          labels=c("Weekd mmay", "Weekend"))
g<-g+labs(y="Average Steps",x = "Time in a day (5-min interval)")
g<-g+ggtitle(paste("Maximum number of steps reported at",timeAtmax$timeOfDay))
print(g)

stepsByTimeWeekday<-subset(stepsByTime,wday=="Weekday")
stepsByTimeWeekend<-subset(stepsByTime,wday=="Weekend")

names(stepsByTimeWeekday)[3]<-"Average.Steps.Weekday"
names(stepsByTimeWeekend)[3]<-"Average.Steps.Weekend"

stepsByTimeWeekday<-select(stepsByTimeWeekday,-wday)
stepsByTimeWeekend<-select(stepsByTimeWeekend,-wday)

stepsByTimenWday<-merge(stepsByTimeWeekday,stepsByTimeWeekend,"Interval")



g<-ggplot(stepsByTimenWday,aes(x=Interval))
g<-g+geom_line(aes(y=Average.Steps.Weekday,color=))
g<-g+geom_line(aes(y=Average.Steps.Weekend))
#g<-g+facet_grid(wday~.)
g<-g+ theme(legend.position="none")
g<-g+labs(y="Average Steps",x = "Time in a day (5-min interval)")
g<-g+ggtitle(paste("Maximum number of steps reported at",timeAtmax$timeOfDay))
